...
class BookingModel {
   ...
   public function getAllBookings() {
        try {
            // SỬA CỘT PEOPLE THÀNH SOLUONGBAN ĐỂ KHỚP VỚI VIEW
            // Đảm bảo lấy cột status
            $sql = "SELECT b.id, b.name, b.phone, b.email, b.soluongban, b.booking_date, b.booking_time, b.branch, b.notes, b.total, b.created_at, b.status 
                    FROM bookings b
                    ORDER BY b.created_at ASC"; // Lấy đơn mới nhất lên đầu
            ...
            // Cần đảm bảo lỗi này được hiển thị trong AdminController
    ...
    public function createBooking($data) {
        try {
            ...
            foreach ($required as $field) {
                if (empty($data[$field])) {
                    // Sử dụng tên key đã được chuyển đổi trong Controller
                    throw new Exception("Thiếu trường bắt buộc: {$field}"); 
                }
            }
            
            // LẤY DỮ LIỆU BỔ SUNG
            // LƯU Ý: Đổi tên cột trong Model từ 'people' sang 'soluongban' nếu DB của bạn sử dụng 'people'
            $userId = $data['id_khach_hang'] ?? null;
            $totalAmount = $data['total'] ?? 0;
            $status = $data['status'] ?? 0; // Lấy status từ $data (sẽ là 0 hoặc 1)
     ...
     * @param int $id ID của đơn đặt bàn
     * @param int $status Trạng thái mới (ví dụ: 1 là Đã xác nhận)
     * @return bool
     */
    ...
    public function deleteBooking($id) {
        try {
            // Xóa chi tiết liên quan trước (giả định booking_items)
